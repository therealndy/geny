...
class ThoughtLoopService {
  ...
  Future<List<String>> reflect(String input, {List<MemoryItem>? recentMemories}) async {
    ...
    if (input.length < 20) reflections.add('Input is short — ask a clarifying question');
    if (input.contains('?')) reflections.add('User asked a question — prioritize answering');
  ...
  final types = <String>{};
  for (var m in recentMemories) types.add(m.type.toString().split('.').last);
  final oldest = recentMemories.last.timestamp;
  final newest = recentMemories.first.timestamp;
  reflections.add('Memory summary: types=${types.join(',')}, newest=${newest.toIso8601String()}, oldest=${oldest.toIso8601String()}');
    }
